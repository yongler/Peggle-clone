...
enum GameMode {
    ...
    static func getIsLose(score: Int, timeLeftInSeconds: Int, board: Board, gameMode: GameMode) -> Bool {
        ...
        case .beatTheScore:
            return score < score && (board.ballsLeftCount == 0 || timeLeftInSeconds <= 0)
        case .siamLeftSiamRight:
            return board.ballsShotCount < board.siamLeftSiamRightBallsCount && ( timeLeftInSeconds > 0 || board.pegsHitCount > 0)
        }
    }
