...
class BucketPhysicsObject: RectangleObject {
    ...
    convenience init(bucket: Bucket) {
        self.init(centre: bucket.centre, width: bucket.width, height: bucket.height, velocity: BucketPhysicsObject.defaultVelocity, acceleration: Acceleration.zero)
    }
    
    func getBucket() -> Bucket {
        return Bucket(centre: self.centre)
    }
}
